...
export class StockService {
  ...
  getCompanyName(symbol: string): Observable<CompanyModel> {
    ...
    return this.http.get(url).pipe(
      map((res: CompanyResponseModel) => {
        const filteredCompany = res.result.filter(
          (x) => x.symbol === symbol
        )[0];
  ...
  getSentimentDetails(
    ...
    fromDate: string,
    toDate: string
  ): Observable<SentimentResponseModel> {
    let url =
      this.configuration.SentimentUrl +
      '?symbol=' +
      symbol +
      '&from=' +
      fromDate +
  ...
  getStockDataFromLocalStorage(): StockDetailsModel[] {
    ...
    let stockDetalsStr: string = localStorage.getItem(
      LocalStorageEnum.STOCK_DATA.toString()
    );
